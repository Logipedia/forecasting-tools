...
class CodaRow:
    ...
    def turn_to_payload_friendly_json(self) -> list[dict]:
        ...
        """
        cell_jsons: list[dict] = [cell.turn_to_payload_friendly_json() for cell in self.cells]
        payload = [
            {
                'cells': cell_jsons
            }
...
class CodaTable:
    ...
    def __init__(self, doc_id: str, table_id: str, columns: list[CodaColumn], key_columns: list[CodaColumn]):
        self.doc_id = doc_id
