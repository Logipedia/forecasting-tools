...
class HardLimitManager:
    _active_limit_managers: ContextVar[list[HardLimitManager]] = ContextVar('_active_limit_managers', default=[])
    _id_counter: int = 0

    def __init__(self, hard_limit: float = 0, log_usage_when_called: bool = False) -> None:
        assert hard_limit >= 0
    ...
    def raise_error_if_limit_would_be_reached(cls, amount_to_check_room_for: float = 0) -> None:
        ...
        for cost_manager in cls._active_limit_managers.get():
            if cost_manager.amount_left < amount_to_check_room_for and cost_manager.hard_limit != 0:
                raise HardLimitExceededError(f"Usage amount {amount_to_check_room_for} would push current usage to {cost_manager.current_usage + amount_to_check_room_for} exceeding the hard limit of {cost_manager.hard_limit}")
    ...
    def increase_current_usage_in_parent_managers(cls, amount: float) -> None:
        ...
        if amount == 0:
            logger.info("The cost inputted is zero which may or may not be a problem")
        for cost_manager in cls._active_limit_managers.get():
            cost_manager._current_usage += amount
            if cost_manager._current_usage > cost_manager.hard_limit and cost_manager.hard_limit != 0:
                logger.warning(f"Usage increase exceeded the hard limit of {cost_manager.hard_limit}")
            if cost_manager.__log_usage_when_called:
                logger.info(f"{cost_manager.__class__}.ID{cost_manager.id}. Current usage now {cost_manager._current_usage}. Cost of {amount} added")
