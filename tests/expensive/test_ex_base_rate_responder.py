...
# [ ] Gives a resonable rate when only resolve date is given but a known historical rate can be found
...
def assert_base_rate_report_parts_exist(base_rate_report: BaseRateReport) -> None:
    ...
    assert isinstance(base_rate_report.historical_rate, float)
    assert isinstance(base_rate_report.markdown_report, str)
    assert isinstance(base_rate_report.numerator_reference_class, ReferenceClassWithCount)
    assert isinstance(base_rate_report.denominator_reference_class, ReferenceClassWithCount)
