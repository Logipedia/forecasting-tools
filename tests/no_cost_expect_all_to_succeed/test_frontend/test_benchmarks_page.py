...
def test_auth_is_enabled() -> None:
    ...
    app_test.run()
    main_page_warnings = app_test.main.warning
    sidebar_warnings = app_test.sidebar.warning
    assert len(main_page_warnings) == 1, "Warning not displayed as expected"
    assert len(sidebar_warnings) == 1, "Warning not displayed as expected"
