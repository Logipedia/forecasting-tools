...
class ProportionStatCaculator:
    ...
    def determine_if_population_proportion_is_above_p0(self, p0: float, desired_confidence: float) -> tuple[float, bool, str]:
        """
        Requirements
        ...
        For an online test caculator see the below (it may or may not use t distribution at lower sample sizes)
        https://www.statssolver.com/hypothesis-testing.html
        """
        
        if self.number_of_trials * p0 < 5 or self.number_of_trials * (1 - p0) < 5:
            raise ValueError("The normal distribution approximation conditions are not satisfied. Too few samples given the desired p0 to test")
        
        sample_proportion = self.number_of_successes / self.number_of_trials
        
        if self.number_of_trials * sample_proportion < 5 or self.number_of_trials * (1 - sample_proportion) < 5:
            raise ValueError("The normal distribution approximation conditions are not satisfied. Sample proportion is too close to 0 or 1")
