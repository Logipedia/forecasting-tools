...
def assert_coroutines_run_under_x_times_duration_of_benchmark(benchmark_coroutines: list[Coroutine], test_coroutines: list[Coroutine], x: int = 2, allowed_errors_or_timeouts: int = 0) -> None:
    ...
    timed_timedout_coroutines = async_batching.wrap_coroutines_with_timeout(timing_coroutines, max_allowed_duration)
    timed_timedout_and_error_handled_coroutines = async_batching.wrap_coroutines_to_return_not_raise_exceptions(timed_timedout_coroutines)

    # Time the async_batching ask_chat
    list_start_time = time.time()
    duration_result_tuples: list[Tuple[float,float,Any] | Exception] = async_batching.run_coroutines(timed_timedout_and_error_handled_coroutines)
    ...
    if len(non_errored_durations) == 0:
        ...
        average_non_errored_duration = sum(non_errored_durations) / len(non_errored_durations)

    # Log the stats
    non_errored_end_times_with_start_time_as_0 = [end_time - list_start_time for end_time in non_errored_end_times]
    ...
    if len(errored_results) > allowed_errors_or_timeouts:
        raise RuntimeError(f"{len(errored_results)} of the coroutines brought up an error while running. Here are the exceptions: {errored_results}")

    # Assert the time is as expected
    assert non_errored_full_duration < max_allowed_duration, f"Duration of coroutines that did not timeout/error was greater than max allowed duration. Actual Duration: {non_errored_full_duration} Max Allowed Duration: {max_allowed_duration} Time of benchmark: {average_benchmark_duration}"
...
def find_stats_of_coroutine_run(coroutines: list[Coroutine]) -> CoroutineTestInfo:
    start_time = time.time()
...
def profile_coroutine_list(coroutine_list: list[Coroutine]) -> pstats.Stats:
    ...
    log_path = file_manipulation.get_absolute_path(f"logs/misc/profile_stats_{coroutine_name}_{time.time()}.txt")
...
def assert_resource_rate_not_too_high_or_too_low(
    ...
    average_resources_per_second = total_resources_used / duration_of_coroutines_in_seconds
    allowed_upper_bound = allowed_resources_per_period / period_length_in_seconds * over_rate_allowed
    allowed_lower_bound = allowed_resources_per_period / period_length_in_seconds * under_rate_allowed
    ...
    assert average_resources_per_second < allowed_upper_bound, f"Resource rate exceeded upper bound of {allowed_upper_bound}. {additional_message}"
    assert average_resources_per_second > allowed_lower_bound, f"Resource rate was lower than lower bound of {allowed_lower_bound}. {additional_message}"
...
def assert_resource_burst_is_short(burst_size: int, duration: float) -> None:
    ...
    expected_time_to_run_all_coroutines = average_time_to_run_a_coroutine_by_computer * burst_size
    target_duration = expected_time_to_run_all_coroutines * 1.1
    assert duration < target_duration, f"The burst took too long. Expected duration: {expected_time_to_run_all_coroutines}, Actual duration: {duration}"
