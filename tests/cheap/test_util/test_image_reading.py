...
class ImageReadingTestInfo:
    ...
    EXAMPLES = [
        TestImageElement(
            EXAMPLE_IMAGE_ELEMENT_1, TEXT_SNIPPETS_FROM_EXAMPLE_IMAGE_1
        ),
        TestImageElement(
            EXAMPLE_IMAGE_ELEMENT_2, TEXT_SNIPPETS_FROM_EXAMPLE_IMAGE_2
        ),
    ]
...
def test_html_images_successfully_replaced_with_descriptions() -> None:
    for example in ImageReadingTestInfo.EXAMPLES:
        ...
        conversion_coroutine = (
            ImageReader.replace_html_images_with_paragraph_descriptions(
                html_content
            )
        )
        ...
        for text_snippet in example.text_snippets:
            assert (
                text_snippet in new_html_content.lower()
            ), "Image content was not identified"
        assert (
            new_html_content.count(ImageReader.IMAGE_DESCRIPTION_START_TAG)
            == 2
        ), "The image description start tag should appear exactly twice"
        assert (
            new_html_content.count(ImageReader.IMAGE_DESCRIPTION_END_TAG) == 2
        ), "The image description end tag should appear exactly twice"
        assert "<img" not in new_html_content, "Image tag was not removed"

        assert (
            new_html_content.lower().count("<p") == 3
        ), "There should be 3 paragraph tags"
        assert (
            new_html_content.lower().count("</p>") == 3
        ), "There should be 3 paragraph tags"
        assert (
            "wooorrrrddd" in new_html_content.lower()
        ), "Other elements were not preserved"
...
def test_nothing_changed_if_no_image_tags() -> None:
    html_content = (
        "<div><p>Other Paragraph with a unique wooorrrrddd</p></div>"
    )
    conversion_coroutine = (
        ImageReader.replace_html_images_with_paragraph_descriptions(
            html_content
    ...
    assert (
        "finance" not in new_html_content.lower()
    ), "Image content was identified"
    assert (
        "buzz" not in new_html_content.lower()
    ), "Image content was identified"
    assert (
        "wooorrrrddd" in new_html_content.lower()
    ), "Other elements were not preserved"
    ...
    assert "<img" not in new_html_content, "Image tag was not removed"
    assert (
        "[Image description start]" not in new_html_content
    ), "Image text start was added"
    assert (
        "[Image description end]" not in new_html_content
    ), "Image text end was added"
