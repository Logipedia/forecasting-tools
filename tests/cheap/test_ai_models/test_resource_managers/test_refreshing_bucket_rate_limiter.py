...
class ResourceLimiterTester:
    ...
    async def function_that_use_random_resources(
        self, resources_to_aquire_range: tuple[int, int]
    ) -> None:
        resources_aquired = random.randint(*resources_to_aquire_range)
        await self.resource_limiter.wait_till_able_to_acquire_resources(
            resources_aquired
        )
        await self.increment_counter(resources_aquired)

    async def function_using_fixed_resources(self, resources_to_aquire: int) -> None:
        await self.resource_limiter.wait_till_able_to_acquire_resources(
            resources_to_aquire
        )
        await self.increment_counter(resources_to_aquire)
...
def test_error_thrown_if_capcity_reached_with_zero_refresh() -> None:
    refresh_rate = 0
    capacity = 1
...
def test_resource_manager_works_with_a_lot_of_random_resource_requests() -> None:
    ...
    assert_whether_rate_limit_is_respected_and_resources_tracked_right(
        tester, coroutines, period_length_in_seconds, allowed_resources_per_period
    )
...
def assert_whether_rate_limit_is_respected_and_resources_tracked_right(
    ...
    total_resources_used = (
        tester.resource_limiter.caculate_resources_passed_into_acquire_in_time_range(
