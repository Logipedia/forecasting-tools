            ...
            if submitted:
                if question_text:
                    with st.spinner(
                        ...
                        await cls.__run_niche_list_research(
                            question_text
                        )
                else:
    ...
    async def __run_niche_list_research(
        ...
        try:
            with MonetaryCostManager() as cost_manager:
                ...
                fact_checked_items = await generator.research_list_of_niche_reference_class(
                    include_incorrect_items=True
                )

                st.subheader("Niche List Research Results")
                markdown = FactCheckedItem.make_markdown_with_valid_and_invalid_lists(
                    fact_checked_items
                )
